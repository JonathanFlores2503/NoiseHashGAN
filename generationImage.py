...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  7 19:08:07 2023
...
def show_from_tensor(tensor, pathDirGeneral, nameNoise_Hash, numeGenerate, save = True):
    img = tensor.clone()
    ...
    plt.imshow(img)
    if (save):
        plt.savefig(pathDirGeneral + "/" + nameNoise_Hash[:8] + "_GAN_" + numeGenerate + ".png", bbox_inches='tight', pad_inches=0)
    plt.show()
...
def showme(Params, show_crop):
  with torch.no_grad():
    ...
    if (show_crop):
      ...
      show_from_tensor(aug_gen_norm, pathDirGeneral, nameNoise_Hash, numeGenerate, save = True)
    print("Generation")
    latest_gen=norm_data(generated.cpu())
    show_from_tensor(latest_gen[0],pathDirGeneral, nameNoise_Hash, numeGenerate,  save = True)
  return (latest_gen[0]) 
...
with torch.no_grad():
    ...
    show_from_tensor(img[0], pathDirGeneral, nameNoise_Hash, numeGenerate, save = False)
